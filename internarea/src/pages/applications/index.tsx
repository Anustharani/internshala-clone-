...
const index = () => {
  ...
  useEffect(() => {
    const fetchdata = async () => {
      try {
        const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL || "${process.env.NEXT_PUBLIC_API_URL || "https://internshala-clone-y2p2.onrender.com/api"}"}/application`);
        setdata(res.data);
      } catch (error) {
        console.log(error);
      }
  ...
  const handleacceptandreject = async (id: any, action: any) => {
    try {
      const res = await axios.put(
        `${process.env.NEXT_PUBLIC_API_URL || "https://internshala-clone-y2p2.onrender.com/api"}/application/${id}`,
        { action }
      );
