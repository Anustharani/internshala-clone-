...
const index = () => {
  ...
  useEffect(() => {
    const fetchdata = async () => {
      try {
        const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL || "https://internshala-clone-y2p2.onrender.com/api"}/job/${id}`);
        setjob(res.data);
      } catch (error) {
        console.log(error);
      }
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 text-black">
      <div className="bg-white rounded-lg shadow-lg overflow-hidden border border-gray-100">
        ...
        <div className="p-6 border-b border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 font-medium text-gray-700">
            <div className="flex items-center space-x-2">
              ...
              <span>{t("location")}: {jobdata.location}</span>
            </div>
            <div className="flex items-center space-x-2">
        ...
        <div className="p-6 border-b border-gray-100">
          ...
          <p className="text-gray-600 leading-relaxed font-medium">{jobdata.aboutCompany}</p>
        </div>
        {/* Job Details Section */}
        <div className="p-6 border-b border-gray-100 space-y-6 text-gray-700 font-medium">
          <div>
            ...
            <p className="text-gray-600 leading-relaxed">{jobdata.aboutJob}</p>
          </div>
          ...
          <div>
            ...
            <p className="text-gray-600 leading-relaxed">{jobdata.whoCanApply}</p>
          </div>

          <div>
            <h3 className="text-lg font-bold text-gray-900 mb-2">{t("perks")}</h3>
            <p className="text-gray-600 leading-relaxed">{jobdata.perks}</p>
          </div>
          ...
          <div>
            ...
            <p className="text-gray-600 leading-relaxed">{jobdata.AdditionalInfo}</p>
          </div>
        </div>
        {/* Apply Button */}
          ...
          <div className="bg-white rounded-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto border border-gray-100 shadow-2xl">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex justify-end pt-4 border-t border-gray-100">
                {user ? (
                  <button
                    className="bg-blue-600 text-white font-bold px-6 py-2.5 rounded-lg hover:bg-blue-700 cursor-pointer shadow-lg shadow-blue-600/10"
